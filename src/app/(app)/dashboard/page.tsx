...
export default function DashboardPage() {
  ...
  async function handleDelete(expense: EnrichedExpense) {
    ...
    toastUndo(`${formatINR(expense.amount)} expense deleted`, async () => {
      ...
      if (!restored.error) load(range, person);
    });
  }
  ...
  return (
    <div className="relative flex flex-col gap-6 pb-10">
      ...
      <AddExpenseSheet open={!!editTarget} onOpenChange={(open) => !open && setEditTarget(null)} editExpense={editTarget} onSaved={() => load(range, person)} />
    </div>
  );
}
